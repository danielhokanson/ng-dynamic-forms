...
describe('DynamicFoundationFormComponent test suite', () => {
    ...
    let fixture: ComponentFixture<DynamicFoundationFormComponent>;
    let component: DynamicFoundationFormComponent;
    ...
    beforeEach(waitForAsync(() => {
        TestBed.configureTestingModule({
            imports: [DynamicFoundationFormComponent],
            providers: [provideNgxMask()]
        }).compileComponents().then(() => {
            const service = TestBed.inject(DynamicFormService);
            formGroup = service.createFormGroup(formModel);

            fixture = TestBed.createComponent(DynamicFoundationFormComponent);
    ...
    it('should render a form control container for each top level model', () => {
        const containers = debugElement.children.filter(child => child.name === 'dynamic-foundation-form-control');
    ...
    it('should populate the components query list of a nested form group', () => {
        const formGroupComponent = debugElement
            .query(By.directive(DynamicFoundationFormGroupComponent)).componentInstance as DynamicFoundationFormGroupComponent;

        expect(formGroupComponent.components.length).toBe(1);
        expect(formGroupComponent.components.first instanceof DynamicFoundationFormControlContainerComponent).toBe(true);
    });

    it('should propagate a custom event from a form control nested in a form group to the form output', () => {
        const containers = debugElement.queryAll(By.directive(DynamicFoundationFormControlContainerComponent));
        const nestedContainer = containers
            .map(container => container.componentInstance as DynamicFoundationFormControlContainerComponent)
